...
def get_pdbbind_ESM_embeddings(folder_path, model="650m"):
    import time
    start = time.time()
...
# %%
epp, epn = get_pdbbind_paths_and_names("/fs/pool/pool-marsot/bindbind/datasets/data/equibind_dataset/PDBBind_processed")

# %%
for pp, pn in zip(epp, epn):
    pdb = pr.parsePDB(protein_path)
    for residue in pdb.getHierView().iterResidues():
        atoms = list(residue.iterAtoms())
        atoms = set(atom.getName() for atom in atoms)
        if "CA" in atoms and any(("C" not in atoms, "N" not in atoms, "O" not in atoms)):
            print(f"problem with {pn}")
...
for pp, pn in zip(epp, epn):
    ...
    for residue in pdb.getHierView().iterResidues():
        ...
        atoms = set(atom.getName() for atom in atoms)
        if "CA" in atoms and any(("C" not in atoms, "N" not in atoms, "O" not in atoms)):
            print(f"problem with {pn}, {residue}")
            break
